...
// The live base/token sit in module state read through getPocketBase/getPocketToken — the data
// layer's pocket ranch client reads them per request, so a watchdog restart (new port, new token)
// lands on the next call without rewiring anything.
...
export const getPocketToken = () => pocketToken;
...
// Boot (or attach to) the on-device backend and record where it answers. Verifies the API answers
// this run's token before recording anything, so a half-started backend never strands the console.
export async function startPocket() {
  ...
  pocketBase = base; pocketToken = token;
...
// down, then running:true with the NEW port/token. Updating the module state here means the data
// layer's next socket-reconnect attempt lands on the live backend instead of hammering the dead
// port forever.
let watching = false;
...
async function watchPocket() {
  ...
  try {
    ...
    await listen('pocket-state', (e) => {
      const p = e.payload || {};
      if (p.running && p.port) { pocketBase = 'http://127.0.0.1:' + p.port; pocketToken = p.token; }
