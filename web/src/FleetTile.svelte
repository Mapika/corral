...
<style>
  .tile { display: grid; grid-template-rows: auto minmax(0, 1fr); min-width: 0; height: 210px; text-align: left; background: var(--frame, #000); border: 0; padding: 0; cursor: pointer; font: inherit; color: var(--text); }
  ...
  .lines { min-height: 0; overflow: hidden; display: flex; flex-direction: column; justify-content: flex-end; padding: 10px 14px 12px; }
  .ln { flex: none; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; font: 11px/1.75 var(--mono); color: var(--text-dim); }
  .ln.user { color: var(--text); }
  .ln.text { color: var(--text-dim); }
  .ln.tool { color: var(--text-faint); }
  .ln.think, .ln.pill { color: var(--text-faint); font-style: italic; }
  .ln.err, .ln.perm { color: var(--alert); }
  .cursor { flex: none; width: 7px; height: 13px; margin-top: 3px; background: var(--text-dim); animation: breathe 1.1s steps(2, start) infinite; }
