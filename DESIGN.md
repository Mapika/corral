# corral — "Ink" design system

> **Runtime source of truth is `web/src/tokens.css`.** Everything below the divider is the
> original monopo saigon brief this system descends from — kept as reference, not what ships.

Ink is the shipped direction (2026-07, picked from three mockups): the monopo monochrome remapped
to an all-dark operator console and sharpened.

- **Ground:** near-black canvas (`--bg` #070708), flush single-surface layout. Panels are not
  outlined and not raised — sections separate by faint `--seam` hairlines and whitespace only.
  `--surface` survives for the composer, inputs, modals, and row hover; `--frame` (#000) for
  immersive insets (code, terminal).
- **Type does the hierarchy:** thin oversized numerals (`--w-thin` 200 at 44px+), 10px uppercase
  labels with .14–.16em tracking, Inter for everything, Cascadia mono for paths/data. No hero, no
  eyebrows, no explanatory copy — the user lives here daily; chrome is noise.
- **Mercury = alive.** The silver metallic flow (`--mercury-flow`, gunmetal → silver) appears only
  on running work: live dots, the running stat numeral (gradient text), and the WebGL atmosphere
  behind the dashboard stat band (frozen liquid metal at rest, flowing while agents run,
  always still under reduced-motion).
- **One warm signal.** `--alert` (#ff5a36) marks errors/attention — dots and small text only,
  never fills or large type. Everything else stays achromatic.
- **Controls:** tonal fills (`--chip`/`--chip-hi`) or bare uppercase text-actions. Never outlined.
  The 75px pill radius remains the only curve; everything else is square. Row-level actions are
  text-actions ("REVIEW", "RESUME"); standalone controls are pill chips; the single primary action
  per surface may fill `--paper`.
- **Terminal palette:** pure-black frame, warm-white ink, silver cursor, restrained ANSI tones
  (see Terminal.svelte THEME).

---

# monopo saigon — Style Reference (original brief)
...
> The "Quick Start" CSS at the bottom is the raw export and is *not* what ships — don't paste it in.
